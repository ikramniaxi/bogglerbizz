                                    ...
                                    <div class="main-card mb-3 ">
                                        <div class="">
                                            <form action="" class="form-inline" id="uForm">
                                                <div class="row">
                                                    <div class="col-6">
                                                        <div class="d-flex " style="flex-direction: column display:flex;justify-content:center;align-item:center">
                                                            <label for="" class="mr-2" style="min-width: 100px;">Select User</label>
                                                            <select name="user" id="user" class="form-control">
                                                                <option value="0">Select User</option>
                                                                <?php foreach ($users as $key => $user): ?>
                                                                <option value="{{ $user->id }}"
                                                                    <?= $request->uid == $user->id ? 'selected' : '' ?>>{{ ucfirst($user->name) }}
                                                                </option>
                                                                <?php endforeach; ?>
                                                            </select>
                                                            <div class="form-group ml-2">
                                                                <button class="btn btn-success ml-2" type="submit">Generate Code</button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="col-6 d-flex " style="justify-content:end">
                                                        <button class="btn btn-primary copyme" type="button">
